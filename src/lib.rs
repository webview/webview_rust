...
mod ffi {
    use crate::{Webview, Window, DispatchFn, BindFn, SizeHint};
    use std::os::raw::*;
...
pub unsafe fn webview_create(debug: bool, window: *mut Window) -> *mut Webview {
    return ffi::webview_create(debug as c_int, window);
}

pub unsafe fn webview_dispatch(w: *mut Webview, f: DispatchFn, arg: *mut c_void) {
    return ffi::webview_dispatch(w, f.into(), arg);
}
...
pub unsafe fn webview_bind(w: *mut Webview, name: &str, f: BindFn, arg: *mut c_void) {
    ...
    return ffi::webview_bind(w, c_name.as_ptr(), f.into(), arg);
}
...
pub unsafe fn webview_return(w: *mut Webview, seq: &str, status: c_int, result: &str) {
    ...
    return ffi::webview_return(w, c_seq.as_ptr(), status, c_result.as_ptr());
}

pub use ffi::{webview_destroy, webview_run, webview_terminate, webview_get_window, webview_set_size};