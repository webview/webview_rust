...
impl Webview {
  pub fn create(debug: bool, window: Option<&mut Window>) -> Webview {
    if let Some(w) = window {
      Webview {
        inner: Arc::new(unsafe { sys::webview_create(debug as c_int, w as *mut Window as *mut _) }),
      }
    } else {
      Webview {
  ...
  pub fn step(&mut self, blocking: bool) -> bool {
    (unsafe { sys::webview_step(*self.inner, blocking as i32) } == 0)
  }
  ...
  pub fn navigate(&mut self, url: &str) {
    let c_url = CString::new(url.as_bytes()).expect("No null bytes in parameter url");
